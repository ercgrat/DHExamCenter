...
function insert_question_block()
{
    ...
    var new_head = document.createElement("h4");
    var new_head_text = document.createTextNode("Question " + question_no + ":");
    new_head.appendChild(new_head_text);
    var new_head_textarea = document.createElement("textarea");
    new_head_textarea.name = "questions[]";
    new_head_textarea.cols = "60";
    new_head_textarea.rows = "3";
    new_div_head.appendChild(new_head);
    new_div_head.appendChild(new_head_textarea);
    
    var new_order_checkbox = document.createElement("label");
    var order_checkbox = document.createElement("input");
    order_checkbox.type = "checkbox";
    order_checkbox.name = "order" + question_no;
    var order_span = document.createElement("span");
    order_span.innerHTML = " Preserve answer order?";
    new_order_checkbox.appendChild(order_checkbox);
    new_order_checkbox.appendChild(order_span);
    
    var new_br = document.createElement("br");
    
    var resource_list = document.getElementsByTagName("select")[0];
    var new_resource_list = resource_list.cloneNode(true);
    new_resource_list.name = "resource" + question_no;
    ...
    location.appendChild(new_order_checkbox);
    location.appendChild(new_br);
    location.appendChild(new_resource_list);
...
function remove_question_block()
{
    ...
    if(answer_lists.length > 1)
    {
        ...
        var resources = answers.previousElementSibling;
        var br = resources.previousElementSibling;
        var order = br.previousElementSibling;
        var head = order.previousElementSibling;
        
        location.removeChild(minus);
        location.removeChild(add);
        location.removeChild(answers);
        location.removeChild(resources);
        location.removeChild(order);
        location.removeChild(br);
